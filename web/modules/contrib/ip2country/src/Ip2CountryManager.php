...
class Ip2CountryManager implements Ip2CountryManagerInterface {
  ...
  public function updateDatabase($registry = 'arin', $md5_checksum = FALSE, $batch_size = 200) {
    ...
    foreach ($ftp_urls as $ftp_file) {
      ...
      if ($txt == FALSE) {
        // Fetch failed.
        $this->loggerFactory->get('ip2country')->warning('File empty or not found on @registry server: @ftp_file', ['@registry' => mb_strtoupper($registry), '@ftp_file' => $ftp_file]);
        return FALSE;
      }
      ...
      if ($md5_checksum) {
        ...
        if ($md5 == FALSE) {
          // Fetch failed.
          $this->loggerFactory->get('ip2country')->warning('File not found on @registry server: @ftp_file.md5', ['@registry' => mb_strtoupper($registry), '@ftp_file' => $ftp_file]);
          return FALSE;
        }
        ...
        if ($md5 != md5($txt)) {
          ...
          $this->connection->schema()->dropTable('ip2country_temp');
          $this->loggerFactory->get('ip2country')->warning('Validation of database from @registry server FAILED. MD5 checksum provided for the @ftp_file registry database does not match the calculated checksum.', ['@registry' => mb_strtoupper($registry), '@ftp_file' => $ftp_file]);
          return FALSE;
        }
      }
    ...
    if ($summary_records == $entries) {
      ...
      try {
        ...
        $txn->rollBack();
        $this->connection->schema()->dropTable('ip2country_temp');
        $this->loggerFactory->get('ip2country')->info('Exception in transaction while swapping in new DB table.', ['exception', $e]);
        return FALSE;
      }
      // Commit transaction.
      unset($txn);
